...
class ClienteRepository
{
    ...
    public Cliente Update(Cliente cliente)
    {
        ...
        var command = connection.CreateCommand();
        command.CommandText = "INSERT INTO Cliente VALUES($clienteid, $endereco, $cidade, $regiao, codigopostal, $pais, $telefone)";
        command.CommandText = "UPDATE Clientes SET endereco = $endereco, cidade = $cidade, regiao = $regiao, codigopostal = $codigopostal, pais = $pais, telefone = $telefone WHERE (clienteid = $clienteid)";
        command.Parameters.AddWithValue("$clienteid", cliente.ClienteId);
        command.Parameters.AddWithValue("$endereco", cliente.Endereco);
        command.Parameters.AddWithValue("$cidade", cliente.Cidade);
        command.Parameters.AddWithValue("$regiao", cliente.Regiao);
    ...
    public void Delete(int id)
    {
        ...
        var command = connection.CreateCommand();
        command.CommandText = "DELETE FROM Clientes WHERE (clienteid = $clienteid)";
        command.Parameters.AddWithValue("$clienteid", id);
